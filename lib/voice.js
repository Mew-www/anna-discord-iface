/* ♥♥ mb-de3-en s100 p50 wg1 a50, but no mbrola support here */

const text2wav = require('text2wav');
const { INSTALLED_LANGUAGES, INSTALLED_VARIANTS } = require('./espeakconf.js');
...
class Voice {
  ...
  constructor(administrative_user_tags) {
    this._voice = 'en+f3';
    this._installed_languages = INSTALLED_LANGUAGES; // {name: lang_code, ...}
    this._installed_variants = INSTALLED_VARIANTS; // [name, ...]
  ...
  async _constructWav(processed_phrase) {
    ...
    const synthesized_wav_uint8 = await text2wav(processed_phrase, {'voice': this._voice});
  ...
  setPronounciation(voicechange_message) {
    const requester = voicechange_message.author;
    const language_name = voicechange_message.content.split(' ')[1];
    if (!this._userCanAdminister(requester)) {
      voicechange_message.reply('You do not have the permission to change voice.');
      return false;
    }
    if (!Object.keys(this._installed_languages).includes(language_name)) {
      voicechange_message.reply(`Language ${language_name} is not installed, available are: ${Object.keys(this._installed_languages).join(', ')}`);
      return false;
    }
    const language_code = this._installed_languages[language_name];
    const variant = this._voice.includes('+') ? this._voice.split('+')[1] : '';
    this._voice = `${language_code}${variant?'+':''}${variant}`;
    return true;
  }

  // Check the requester may administer
  setVariant(voicechange_message) {
    const requester = voicechange_message.author;
    const variant = voicechange_message.content.split(' ')[1];
    if (!this._userCanAdminister(requester)) {
      voicechange_message.reply('You do not have the permission to change voice.');
      return false;
    }
    if (!this._installed_variants.includes(variant)) {
      voicechange_message.reply(`Voice ${variant} is not installed, available are: ${this._installed_variants.join(', ')}`);
      return false;
    }
    const language_code = this._voice.split('+')[0];
    this._voice = `${language_code}+${variant}`;
    return true;
  }
